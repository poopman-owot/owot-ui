...
`
  ...
  paste_preview.addEventListener('click', () => {
    ...
    ppc.classList.toggle("hidden", !showPastePreview)
  })
  ...
  var ppc = owot.cloneNode();
  var previewText = "";
  var previewColor = "";
  var previewBG = "";
  var previewprotections = [];
  ppc.id = "ppc"

  ppc = elm.main_view.appendChild(ppc);
  ppc.classList.add("hidden");

  ppCtx = ppc.getContext("2d");
  ppCtx.fillStyle = textRenderCtx.fillStyle;
  ppCtx.strokeStyle = textRenderCtx.strokeStyle;
  ppCtx.lineWidth = textRenderCtx.lineWidth;
  ppCtx.font = textRenderCtx.font;
  ppCtx.textAlign = textRenderCtx.textAlign;
  ppCtx.textBaseline = owotCtx.textBaseline;
  ...
  w.on("cursorMove", function(e) {
    ...
    ppc.style.left = cpX + "px";
    ppc.style.top = cpY + "px";
  ...
  const writeTextToPCC = (text, color, bg, w, h, protections) => {
    if (text == previewText && YourWorld.Color == previewColor && YourWorld.BgColor == previewBG && protections == previewprotections) {
      return
    } else {
      previewText = text;
      previewColor = YourWorld.Color;
      previewBG = YourWorld.BgColor;
      previewprotections = protections
    }
    ppCtx.clearRect(0, 0, ppc.width, ppc.height);
    ...
    for (let i = 0; i < text.length; i++) {
      ...
      const char_color = rgbToHex(int_to_rgb(color[i]));
      const char_bg = rgbToHex(int_to_rgb(bg[i]));
      if (character !== "\x1b") {
        if (bg[i] == -1) {
          ppCtx.fillStyle = ppCtx.fillStyle = rgbToHex(int_to_rgb(YourWorld.BgColor));;
        } else {
          ppCtx.fillStyle = char_bg;
        }
        if (protections[i] == 0 || state.userModel.is_member && protections[i] <= 1 || state.userModel.is_operator || state.userModel.is_owner || true) { //its not working well so its auto on
          if (character !== "\n" && character !== "\r" && character !== "") {
            ppCtx.fillRect(x, y - h, w, h);
          }
          if (color[i] == 0 || color[i] == -1) {
            ppCtx.fillStyle = rgbToHex(int_to_rgb(YourWorld.Color));
          } else {
            ppCtx.fillStyle = char_color;
          }
          ppCtx.fillText(character, x, y - (h * 0.25));
          ppCtx.clearRect(x + w, y - h, w, h);
        }
        x += w;
      ...
      if (result.state === 'granted' || result.state === 'prompt') {
        navigator.clipboard.readText()
          .then(text => {
            ...
            data = textcode_parser(text, {
              tileX: a,
              tileY: b,
              charX: c,
              charY: d
            });
            while ((ch = data.nextItem()) !== -1) {
              pt_char += ch.char;
              pt_color.push(ch.color)
              pt_bg.push(ch.bgColor)
              protections.push(getCharInfo(ch.tileX, ch.tileY, ch.charX, ch.charY).protection);
            }



            writeTextToPCC(pt_char, pt_color, pt_bg, cellW, cellH, protections)
